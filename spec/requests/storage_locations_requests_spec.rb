    ...
    describe "GET #index" do
      ...
      context "csv" do
        ...
        it "includes headers followed by alphabetized item names" do
          ...
          inactive_item = create(:item, name: 'inactive item', active: false) # inactive items are included in the export
          ...
          expect(response.body.split("\n")[0]).to eq([StorageLocation.csv_export_headers, item3.name, item2.name, item1.name, inactive_item.name].join(','))
        end
        ...
        it "Generates csv with Storage Location fields, alphabetized item names, item quantities lined up in their columns, and zeroes for no inventory" do
          ...
          csv = <<~CSV
            Name,Address,Square Footage,Warehouse Type,Total Inventory,A,B,C,D,inactive item
            Storage Location with Duplicate Items,"1500 Remount Road, Front Royal, VA 22630",100,Residential space used,1,0,0,1,0,0
            Storage Location with Items,123 Donation Site Way,100,Residential space used,3,1,1,1,0,0
            Storage Location with Unique Items,Smithsonian Conservation Center new,100,Residential space used,5,0,0,0,5,0
            Test Storage Location,123 Donation Site Way,100,Residential space used,0,0,0,0,0,0
            Test Storage Location 1,123 Donation Site Way,100,Residential space used,0,0,0,0,0,0
          CSV
        ...
        context "when include inactive storage locations checkbox is checked" do
          ...
          it "generates csv with Storage Locations that are inactive" do
            ...
            csv = <<~CSV
              Name,Address,Square Footage,Warehouse Type,Total Inventory,A,B,C,D,inactive item
              Inactive Storage Location,123 Donation Site Way,100,Residential space used,3,1,1,1,0,0
              Storage Location with Duplicate Items,"1500 Remount Road, Front Royal, VA 22630",100,Residential space used,1,0,0,1,0,0
              Storage Location with Items,123 Donation Site Way,100,Residential space used,3,1,1,1,0,0
              Storage Location with Unique Items,Smithsonian Conservation Center new,100,Residential space used,5,0,0,0,5,0
              Test Storage Location,123 Donation Site Way,100,Residential space used,0,0,0,0,0,0
